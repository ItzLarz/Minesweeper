...
function draw() {
  ...
  if (!gameOver && !homeScreenState && gamePlaying) {
    if (millis() - interval >= 1000) {
      ...
      text(minutes + ":" + spacer + seconds, edgeSize + (boxSize * columns) / 1.2, (topBorderSize + edgeSize) / 1.4);
    }
  }
}
...
function gameOverScreen() {
  ...
  if (defeat) {
    ...
    fill(0);
    strokeWeight(0);
    text("Try again?", edgeSize + (boxSize * columns) / 2, (topBorderSize + edgeSize) / 2);
    textAlign(LEFT);
  }
  ...
  if (win) {
    ...
    fill(0);
    strokeWeight(0);
    text("You won!", edgeSize + (boxSize * columns) / 2, (topBorderSize + edgeSize) / 2);
    textAlign(LEFT);
  }
}
...
function drawBorders() {
  ...
  text("Reset", edgeSize + (boxSize * columns) / 2, (topBorderSize + edgeSize) / 2);

  textSize(columns);

  text("Bombs:", edgeSize + (boxSize * columns) / 6, (topBorderSize + edgeSize) / 2.5);
  text(bombCount, edgeSize + (boxSize * columns) / 6, (topBorderSize + edgeSize) / 1.4);
  text("Time:", edgeSize + (boxSize * columns) / 1.2, (topBorderSize + edgeSize) / 2.5);
  text("0:00", edgeSize + (boxSize * columns) / 1.2, (topBorderSize + edgeSize) / 1.4);
...
function sliderChange() {
  ...
  if (topBorderSize < 80) { topBorderSize = 80; }
  if (bottomBorderSize < 40) { bottomBorderSize = 40; }
